...
export default async function GetArtworksList(
  ...
  const url: string = `${BASE_URL}/artworks?limit=${limit}&page=${page}&fields=id,title,artist_display,image_id`;  const response = await fetch(url);
  if (!response.ok) {
    throw new Error(response.statusText);
  }
  const content: ListResponse = await response.json();
  return content.data.map((art: any) => ({
    ...art,
